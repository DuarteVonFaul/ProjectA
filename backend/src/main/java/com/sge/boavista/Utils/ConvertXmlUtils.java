...
public class ConvertXmlUtils {
    ...
    private static String criarTag(String nomeTag, Object valor) {
    ...
    private static String criarTagValor(String nomeTag, BigDecimal valor) {
        String valorFormatado = (valor == null) ? "0.00" : String.format("%.2f", valor);
        return String.format("        <%s>%s</%s>\n", nomeTag, valorFormatado.replace(',','.'), nomeTag);
    }

    public static String gerarXmlRemessaVendaSimples(List<VendaDTO> vendas, LocalDate dataVenda) {
